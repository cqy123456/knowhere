...
class HDF5Reader {
 public:
    ...
    GetKNNGT() {
        int32_t topk, nq;
        auto gt_ids_int32 =
            reinterpret_cast<int32_t*>(hdf5_read(file_name_, HDF5_DATASET_NEIGHBORS, H5T_INTEGER, topk, nq));
        int32_t topk_tmp, nq_tmp;
        auto gt_dis =
            reinterpret_cast<float*>(hdf5_read(file_name_, HDF5_DATASET_DISTANCES, H5T_FLOAT, topk_tmp, nq_tmp));
        ...
        gt->SetRows(nq);
        gt->SetDim(topk);
        gt->SetIds(gt_ids_int64);
        gt->SetDistance(gt_dis);
        delete[] gt_ids_int32;
        return gt;
    }
