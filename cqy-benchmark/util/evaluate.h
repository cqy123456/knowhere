...
private:
  Clock::time_point epoch;
    ...
    GetAverageAndTP99Latancy(std::vector<double>& latancy) {
        auto num = latancy.size();
        auto average = std::accumulate(latancy.begin(), latancy.end(), 0) / (double)num;
        std::sort(latancy.begin(), latancy.end());
        auto tp99_id = int(num * 0.99);
        tp99_id = (tp99_id == num ? num - 1 : tp99_id);
